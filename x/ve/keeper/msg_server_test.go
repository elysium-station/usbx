...
import (
	...
	"github.com/elysium-station/blackfury/x/ve/keeper"
...
func (suite *KeeperTestSuite) TestVeCreate() {
	...
	testCases := []struct {
		...
		{"invalid denom", false, sender, receiver.String(), "1fury", types.RegulatedPeriod},
		{"invalid sender", false, []byte("xxx"), receiver.String(), denom, types.RegulatedPeriod},
		{"invalid receiver", false, sender, "xxx", "1afury", types.RegulatedPeriod},
...
func (suite *KeeperTestSuite) TestVeDeposit() {
	...
	testCases := []struct {
		...
		{"invalid denom", false, sender, sdk.NewCoin("fury", sdk.NewInt(1)), "ve-100"},
