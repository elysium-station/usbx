...
func (k Keeper) totalSupply(ctx sdk.Context, contract common.Address) (*big.Int, error) {
	erc20 := contracts.ERC20BurnableContract.ABI

	// Name
	res, err := k.CallEVM(ctx, erc20, types.ModuleAddress, contract, "totalSupply")
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, sdkerrors.Wrapf(types.ErrABIUnpack, "failed to unpack totalSupply: %s", err)
	}
	if len(unpacked) > 0 {
	...
	return nil, sdkerrors.Wrapf(types.ErrABIUnpack, "failed to unpack totalSupply")
}
...
func (k Keeper) balanceOf(ctx sdk.Context, contract, account common.Address) (*big.Int, error) {
	...
	// Name
	res, err := k.CallEVM(ctx, erc20, types.ModuleAddress, contract, "balanceOf", account)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, sdkerrors.Wrapf(types.ErrABIUnpack, "failed to unpack balanceOf: %s", err)
	}
	if len(unpacked) > 0 {
	...
	return nil, sdkerrors.Wrapf(types.ErrABIUnpack, "failed to unpack balanceOf")
}
