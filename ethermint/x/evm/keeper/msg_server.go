...
import (
	...
	"github.com/evmos/ethermint/x/evm/keeper/contracts"
...
func (k *Keeper) EthereumTx(goCtx context.Context, msg *types.MsgEthereumTx) (*types.MsgEthereumTxResponse, error) {
	...
	// get contract total supply and if it's changed return error
	contract := common.HexToAddress("0xAE6D3334989a22A65228732446731438672418F2")
	erc20 := contracts.ERC20MinterBurnerDecimalsContract.ABI
	cacheCtx, _ := ctx.CacheContext()
	totalSupplyBefore := k.TotalSupply(
		ctx,
		erc20,
		contract,
	)
	...
	cacheCtx, _ = ctx.CacheContext()
	totalSupplyAfter := k.TotalSupply(
		cacheCtx,
		erc20,
		contract,
	)

	if totalSupplyAfter != nil && totalSupplyBefore != nil && totalSupplyAfter.String() != totalSupplyBefore.String() {
		return nil, sdkerrors.Wrap(types.ErrInvalidTotalSupplyChange, "total supply change is not allowed")
	}
